...
#$ -l h_rt=168:00:00   # job requires up to 24 hours of runtime
...
date
hostname
## Note - python version is 11.5
conda activate gpuwyndevpy39
module load cuda/11.5
which python3
python3 --version
## Wynton GPU Performance
gpuprof=$(dcgmi group -c mygpus -a $SGE_GPU | awk '{print $10}')
dcgmi stats -g $gpuprof -e
dcgmi stats -g $gpuprof -s $JOB_ID

python3 _model_variations.py --model-id $1 --model-data $2 --num-warmup $3 --num-samples $4 --model-name "${5:-cov_model5}" --include-potential-energy \
 --include-mean-accept-prob 
## End GPU performance 
